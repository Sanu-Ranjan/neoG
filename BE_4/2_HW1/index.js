...
const createRes = async (restaurantData) => {
  try {
    const data = await Restaurant.create(restaurantData);
...
app.post("/restaurants", async (req, res) => {
  ...
  const { data, error } = await createRes(restaurantData);

  if (error) {
    console.log("Db error: error creating restaurant", error);
    return res.status(500).json(fail("Internal server error", error.message));
  }

  res.status(201).json(success("Restaurant created", data));
});
