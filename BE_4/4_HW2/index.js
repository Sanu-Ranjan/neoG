...
const updateHotel = async (queryParam, updateData) => {
  try {
    const data = await Hotel.findOneAndUpdate(queryParam, updateData, {
      new: true,
    });
...
app.post("/hotels/:hotelId", async (req, res) => {
  ...
  const { data, error } = await updateHotel({ _id: hotelId }, req.body);

  if (error) {
    console.log("Db error: updating hotel", error);
  ...
  res.status(200).json(success("Hotel updated", data));
});
